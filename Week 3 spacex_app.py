...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df[(filtered_df['Launch Site'] == entered_site)].groupby('class')['Flight Number'].count().reset_index(), values='Flight Number', 
        names='class', 
        title='Success Rate')
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site,selected_range):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df[(filtered_df['Payload Mass (kg)'] >= selected_range[0]) & (filtered_df['Payload Mass (kg)'] <= selected_range[1])], x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return fig
    else:
        filtered_df = filtered_df[(filtered_df['Launch Site'] == entered_site) & (filtered_df['Payload Mass (kg)'] >= selected_range[0]) & (filtered_df['Payload Mass (kg)'] <= selected_range[1])]
